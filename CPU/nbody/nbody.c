...
int main(const int argc, const char** argv) 
{
  ...
  p.x = buf+0*nParticles; p.y = buf+1*nParticles; p.z = buf+2*nParticles; p.vx = buf+3*nParticles; p.vy = buf+4*nParticles; p.vz = buf+5*nParticles;
  ...
  // Propagate particles
  printf("Propagating particles using %d threads...\n", omp_get_max_threads()); double rate = 0.0, dRate = 0.0; // Benchmarking data
  const int skipSteps = 1; // Skip first iteration is warm-up on Xeon Phi coprocessor i
  const float eps2 = 0.001*0.001;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < nParticles; i++) { // Parallel loop over particles that experience force 
      ...
      for (int j = 0; j < nParticles; j++) { // Vectorized loop over particles that exert force 
        ...
        const float drSquared = dx*dx + dy*dy + dz*dz + eps2;
        const float drPowerN32 = drSquared * drSquared;//1.0f/(drSquared*sqrtf(drSquared));
  ...
  rate/=(double)(nSteps-skipSteps); dRate=sqrt(dRate/(double)(nSteps-skipSteps)-rate*rate); printf("Average rate for iterations %d through %d: %.3f +- %.3f steps per second.\n",
